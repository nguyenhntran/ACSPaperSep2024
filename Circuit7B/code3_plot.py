...
# Load data
data0 = np.load('circuit2_Paretos_Sens1_final.npz', allow_pickle=True)
data1 = np.load('circuit2_Paretos_Param1_final.npz', allow_pickle=True)
...
pairs = [
    (
        data0[f"paretoset_Sens1Pair{i}_final"],
        data1[f"paretoset_Sens1mask{i}_Param_final"]
    )
    for i in range(1, 16)
]
...
for i, ((sens_data_pareto, param_data), (xlabel, ylabel)) in enumerate(zip(pairs, labels_2d), 1):
    ...
    ax_3d.locator_params(nbins=4, axis='x')  # X-axis: 5 ticks
    ax_3d.locator_params(nbins=4, axis='y')  # Y-axis: 5 ticks
    ...
    # Set title and save
    fig.suptitle(
        f'Pareto-Optimal Front and Parameters {i}',
        fontsize=10, fontweight='bold'
    )
    plt.savefig(f'Paretos_Sens1Pair{i}_SS1.png', dpi=300)
    plt.close(fig)
