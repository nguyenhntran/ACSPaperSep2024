...
# Load data
data0 = np.load('Toggle_Paretos_Sens2_final.npz', allow_pickle=True)
data1 = np.load('Toggle_Paretos_Param2_final.npz', allow_pickle=True)
...
for i, ((sens_data, param_data), (xlabel, ylabel)) in enumerate(zip(pairs, labels_2d), 1):
    ...
    # Set title and save
    fig.suptitle(
        f'Double Negative Feedback Steady State 2 L=0.1:\nPareto-Optimal Fronts and Parameters Pair {i}',
        fontsize=12, fontweight='bold'
    )
    plt.savefig(f'Toggle_Paretos_L0.1_Sens1Pair{i}_SS2.png', dpi=300)
    plt.close(fig)
