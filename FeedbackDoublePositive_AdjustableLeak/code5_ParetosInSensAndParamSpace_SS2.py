...
# Load data
data0 = np.load('dpos_Paretos_Sens2_final.npz', allow_pickle=True)
data1 = np.load('dpos_Paretos_Param2_final.npz', allow_pickle=True)
globals().update(data0)
globals().update(data1)

# Define pairs of sensitivity and parameter data
pairs = [
    (globals()[f"paretoset_Sens2Pair{i}_final"], globals()[f"paretoset_Sens2mask{i}_Param_final"])
...
for i, ((sens_data, param_data), (xlabel, ylabel)) in enumerate(zip(pairs, labels_2d), 1):
    ...
    # 2D scatter plot
    axes[0].scatter(sens_data[:, 0], sens_data[:, 1], s=10)
    ...
    # Set title and save
    fig.suptitle(
        f'Double Positive Feedback Steady State 2 L=1:\nPareto-Optimal Fronts and Parameters Pair {i}',
        fontsize=12, fontweight='bold'
    )
    plt.savefig(f'dpos_Paretos_L1_Sens1Pair{i}_SS2.png', dpi=300)
    plt.close(fig)
