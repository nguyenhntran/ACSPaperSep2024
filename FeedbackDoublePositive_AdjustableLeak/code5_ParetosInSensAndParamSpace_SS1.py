...
# Load data
data0 = np.load('dpos_Paretos_Sens1_final.npz', allow_pickle=True)
data1 = np.load('dpos_Paretos_Param1_final.npz', allow_pickle=True)
...
for i, ((sens_data, param_data), (xlabel, ylabel)) in enumerate(zip(pairs, labels_2d), 1):
    ...
    # Set title and save
    fig.suptitle(
        f'Double Positive Feedback Steady State 1 L=1:\nPareto-Optimal Fronts and Parameters Pair {i}',
        fontsize=12, fontweight='bold'
    )
    plt.savefig(f'dpos_Paretos_L1_Sens1Pair{i}_SS1.png', dpi=300)
    plt.close(fig)
